...
log_print("\n=== 岗位分布 ===", df["role_type"].value_counts())
...
# ===== 5. 分布 =====
log_print("\n=== remote_flag分布 ===", df["remote_flag"].value_counts())
...
# 查看分布
log_print("\n=== 经验年数分布 ===", df["experience_years"].value_counts())
...
# ===== 打印分布情况 =====
log_print("\n=== Level 分布统计 ===", df["level"].value_counts())
...
skill_summary_df  = pd.DataFrame({
    '出现次数': df[exp_cols].sum(),
    '出现频率 (%)': (df[exp_cols].mean() * 100).round(2)
})

# 3. 按出现次数从高到低排序，让你一眼看到“最热门技能”
skill_summary_df  = skill_summary_df .sort_values(by='出现次数', ascending=False)

log_print("--- 技能关键词统计分析 ---", skill_summary_df)


# 2. 定义【工作内容】特征字典 (Key 为列名, Value 为匹配词列表)
job_content_dict = {
    # 数据工程与可视化
    "job_数据清洗": ["データクレンジング", "データ整備", "データ加工", "データマネジメント"],
    "job_可视化": ["ビジュアライズ", "レポーティング", "ダッシュボード", "可視化", "見える化"],
    "job_基盘构建": ["データ基盤", "データベース構築", "マスターデータ", "DWH", "ETL"],
    
    # 分析与算法
    "job_统计分析": ["统计解析", "傾向集計", "定量分析", "定性分析", "インサイト"],
    "job_算法实现": ["アルゴリズム", "予測モデル", "予兆保全", "機械学習", "AI解析"],
    
    # 业务与战略
    "job_战略立案": ["戦略立案", "グランドデザイン", "事業成長", "中長期計画"],
    "job_课题解决": ["課題解決", "変革", "コンサルティング", "提案", "ソリューション"],
    "job_KPI管理": ["KPI設計", "モニタリング", "PDCA", "効果検証", "数値管理"],
    "job_营销CRM": ["CRM", "LTV", "リードナーチャリング", "マーケティング施策"],
    
    # 数字化转型与开发
    "job_DX推进": ["DX", "デジタルトランスフォーメーション", "デジタル化", "IT戦略"],
    "job_IoT相关": ["IoT", "センサー", "遠隔監視", "産業機器", "稼働データ"],
    "job_系统设计": ["要件定義", "システム開発", "インフラ刷新", "設計・構築"],
    
    # 市场与协作
    "job_市场调查": ["市場分析", "競合分析", "市場調査", "調査研究"],
    "job_品牌推广": ["ブランディング", "プロモーション", "広告", "商品企画"],
    "job_部门协作": ["跨部门", "クロスファンクショナル", "連携", "調整", "ステークホルダー"]
}
...
def apply_one_hot(text, keywords):
    ...
    for word in keywords:
        # 使用 regex=False 提高速度，如果需要精确匹配单词边界可参考之前的正则写法
        if word in str(text):
            return 1
    return 0
...
job_summary_df = pd.DataFrame({
    '出现次数': df[job_cols].sum(),
    '出现频率 (%)': (df[job_cols].mean() * 100).round(2)
})

# 3. 按出现次数从高到低排序，让你一眼看到“最热门技能”
job_summary_df = job_summary_df.sort_values(by='出现次数', ascending=False)

log_print("--- 工作内容统计分析 ---", job_summary_df)
